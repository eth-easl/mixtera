...
class TestPropertyCalculationExecutor(unittest.TestCase):
    ...
    def test_from_mode_raises_error_with_dop_less_than_1(self):
        with self.assertRaises(RuntimeError) as context:
            PropertyCalculationExecutor.from_mode(ExecutionMode.LOCAL, "test", 0, 10, Mock(), Mock())
        self.assertEqual(str(context.exception), "dop = 0 < 1")

    def test_from_mode_raises_error_with_batch_size_less_than_1(self):
        with self.assertRaises(RuntimeError) as context:
            PropertyCalculationExecutor.from_mode(ExecutionMode.LOCAL, "test", 1, 0, Mock(), Mock())
        self.assertEqual(str(context.exception), "batch_size = 0 < 1")

    def test_from_mode_returns_local_executor_instance(self):
        setup_func = Mock()
        calc_func = Mock()
        executor = PropertyCalculationExecutor.from_mode(ExecutionMode.LOCAL, "test", 1, 10, setup_func, calc_func)
    ...
    def test_from_mode_raises_not_implemented_for_unknown_mode(self):
        with self.assertRaises(NotImplementedError) as context:
            PropertyCalculationExecutor.from_mode("UNKNOWN_MODE", "test", 1, 10, Mock(), Mock())
