...
class TestServerConnection(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_fetch_file_async(
        ...
        file_data = await self.server_connection._fetch_file(filesys_id, file_path)
        ...
        mock_write_int.assert_has_calls(
            [
                ...
                call(filesys_id, ID_BYTES, mock_writer, drain=False),
            ]
        )
    ...
    def test_get_file_iterable_sync(self, mock_fetch_file):
        ...
        file_iterable = self.server_connection.get_file_iterable(filesys_id, file_path)
        lines = list(file_iterable)

        self.assertEqual(lines, ["line1", "line2", "line3"])
        mock_fetch_file.assert_awaited_once_with(filesys_id, file_path)
