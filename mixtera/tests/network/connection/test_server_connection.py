...
class TestServerConnection(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_query_result_meta_async(
        ...
        meta_result = await self.server_connection._get_query_result_meta(training_id)
        ...
        mock_write_string.assert_has_calls([call(training_id, ID_BYTES, mock_writer)])
    ...
    def test_get_query_results_sync(self, mock_get_next_result):
        ...
        results = self.server_connection._stream_result_chunks(training_id)
        result_list = list(results)

        self.assertEqual(result_list, [[1, 2, 3], [4, 5, 6]])
        mock_get_next_result.assert_has_calls([call(training_id), call(training_id), call(training_id)])
    ...
    async def test_get_next_result(
        ...
        result_chunk = await self.server_connection._get_next_result(training_id)
